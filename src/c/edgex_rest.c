...
static edgex_deviceservice *deviceservice_read (const JSON_Object *obj)
{
  ...
  result->adminState = get_string (obj, "adminState");
  result->created = json_object_get_number (obj, "created");
  result->description = get_string (obj, "description");
  result->id = get_string (obj, "id");
  result->labels = array_to_strings (json_object_get_array (obj, "labels"));
...
deviceservice_write (const edgex_deviceservice *e, bool create)
{
  ...
  json_object_set_string (obj, "adminState", e->adminState);
  json_object_set_string (obj, "description", e->description);
  json_object_set_value (obj, "labels", strings_to_array (e->labels));
...
static edgex_reading *reading_read (const JSON_Object *obj)
{
  edgex_reading *result = malloc (sizeof (edgex_reading));
  result->created = json_object_get_number (obj, "created");
  result->id = get_string (obj, "id");
  result->modified = json_object_get_number (obj, "modified");
  result->name = get_string (obj, "name");
  result->origin = json_object_get_number (obj, "origin");
  result->pushed = json_object_get_number (obj, "pushed");
  result->value = get_string (obj, "value");
  result->next = NULL;

  return result;
}

static JSON_Value *reading_write (const edgex_reading *e, bool create)
{
  JSON_Value *result = json_value_init_object ();
  JSON_Object *obj = json_value_get_object (result);
  ...
  if (!create)
  {
    json_object_set_number (obj, "created", e->created);
    json_object_set_number (obj, "modified", e->modified);
    json_object_set_number (obj, "pushed", e->pushed);
  }

  json_object_set_string (obj, "id", e->id);
  json_object_set_string (obj, "name", e->name);
  json_object_set_number (obj, "origin", e->origin);
  json_object_set_string (obj, "value", e->value);
...
static edgex_event *event_read (const JSON_Object *obj)
{
  edgex_event *result = malloc (sizeof (edgex_event));
  result->created = json_object_get_number (obj, "created");
  result->device = get_string (obj, "device");
  result->id = get_string (obj, "id");
  result->modified = json_object_get_number (obj, "modified");
  result->origin = json_object_get_number (obj, "origin");
  result->pushed = json_object_get_number (obj, "pushed");
  JSON_Array *readings_array = json_object_get_array (obj, "readings");
  size_t readings_count = json_array_get_count (readings_array);
  for (size_t i = 0; i < readings_count; i++)
  {
    edgex_reading **readings_last = &result->readings;
    edgex_reading *temp = reading_read
      (json_array_get_object (readings_array, i));
    *readings_last = temp;
    readings_last = &(temp->next);
  }

  return result;
}

static JSON_Value *event_write (const edgex_event *e, bool create)
{
  JSON_Value *result = json_value_init_object ();
  JSON_Object *obj = json_value_get_object (result);
  ...
  json_object_set_string (obj, "id", e->id);
  json_object_set_number (obj, "origin", e->origin);
  JSON_Value *readings_array_val = json_value_init_array ();
  JSON_Array *readings_array = json_value_get_array (readings_array_val);
  for (edgex_reading *temp = e->readings; temp; temp = temp->next)
  {
    json_array_append_value (readings_array, reading_write (temp, create));
  }
  json_object_set_value (obj, "readings", readings_array_val);

  return result;
}

static JSON_Value *events_write (const edgex_event *e, bool create)
{
  JSON_Value *result = json_value_init_array ();
  JSON_Array *array = json_value_get_array (result);
  ...
  while (e)
  {
    json_array_append_value (array, event_write (e, create));
    e = e->next;
  }
...
void edgex_event_free (edgex_event *e)
{
  while (e)
  {
    edgex_event *current = e;
    free (e->device);
    free (e->id);
    edgex_reading_free (e->readings);
    e = e->next;
    free (current);
  }
}
